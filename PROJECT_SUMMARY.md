...
| 6 字段 design（数学抽象易成词汇包装）| **16 字段 formal method contract**（objective/likelihood/identifiability/复杂度/uncertainty）|
...
| **algorithm-design** | 算法设计 | **16 字段 formal method contract**（problem/estimand/abstraction/notation/assumptions/**objective_or_likelihood**/**identifiability**/cross-domain/algorithm/**optimization_or_inference**/**complexity**/failure_boundaries/uncertainty/invariances/simulation_dgp/**benchmark_protocol**/novelty_or_utility）；**"utility first, novelty explicit"** stance；自主跑 + 推理外显 |
...
| `_shared/artifact-schema.json` 🆕 | artifact 结构 | JSON Schema（id/parent/stage/provenance_hash/stage_fields）|
...
data-and-estimand-audit ✋GATE  (fatal_issues 非空 = 停)
  ...
  → algorithm-design  (16字段 formal method; utility-first; 自主跑+推理外显)
...
├── README.md  CLAUDE.md  PROJECT_SUMMARY.md  LICENSE  .gitignore
├── skills/
│   ├── using-crossbio-algo/SKILL.md
│   ├── data-and-estimand-audit/SKILL.md          🆕 v0.2
│   ├── brainstorm/SKILL.md
│   ├── topic-viability-assessment/SKILL.md
│   ├── algorithm-design/{SKILL.md, cross-domain-inspiration.md}
│   ├── spec-writing/SKILL.md
│   ├── adversarial-panel-audit/{SKILL.md, agents/*.md (6)}  🔄 v0.2 改名+agents
│   └── _shared/{research-design-handoff.md, artifact-schema.json, artifact-validation.md}
├── examples/scout/  (spec + 代码 + 8 测试含 mutation)
└── tests/baseline-tests.md  (6 skill RED+GREEN)
```
...
> crossbio-algo v0.2 把"自主科研 agent"最有价值的部分（科研诚实纪律 + 数据审计 + formal method + 机器可校验 handoff + 对抗 panel）提炼成一个 7-skill Claude Code 闭环——从数据审计到可执行 spec，带 GATE/竞品查证/双向审计/失效边界/三档模式。v0.1→v0.2 的核心转变：从 "idea novelty pipeline" 到 **"evidence-driven + machine-checkable"**。评审建议基本全落实（8 commit）。待 GitHub 发布 + 多方向验证。
