...
crossbio_validate/schemas/stage-schemas.json    CANONICAL machine schema ($defs + oneOf) — single source of truth
...
Every stage emits an `artifact.json` conforming to `crossbio_validate/schemas/stage-schemas.json`. Artifacts link via `parent_artifact_id` and carry a `provenance_hash`. The chain is checked by the 5 cross-stage rules (estimand continuity / no-orphan failure_boundary / notation consistency / pseudocode→code / provenance).
...
- **Field lists must stay consistent with `crossbio_validate/schemas/stage-schemas.json`.** If you add/rename a required field in a skill's output contract, update the matching `$defs` entry (and the validator tests) in the same change. The schema is the single source of truth — don't restate a divergent field list in prose.
...
- If you changed a skill, its frontmatter still parses and its field list still matches `crossbio_validate/schemas/stage-schemas.json`.
